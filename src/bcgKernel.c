...
void bcg_v1(csrType_local mat, denseType B, denseType X, double epsilon, int myid, int numprocs) {
    ...
    int mat_global_row_num = mat.num_cols;
    int mat_global_col_num = mat.num_cols;
    ...
    int X_num_data = X.local_num_col * X.local_num_row;
    ...
    ierr = MPI_Reduce((void*) local_norm, (void*) global_norm, A_X.global_num_col
    ...
    int ind_array[2] = {0, B.local_num_col * B.local_num_row - 1};
    ...
    int bcg_v1_debug_2_idx_i;
    int bcg_v1_debug_2_idx_j;
    for (bcg_v1_debug_2_idx_i = 0; bcg_v1_debug_2_idx_i < R.local_num_row; bcg_v1_debug_2_idx_i++)
        for (bcg_v1_debug_2_idx_j = 0; bcg_v1_debug_2_idx_j < R.local_num_col; bcg_v1_debug_2_idx_j++) {
            int RP_idx = bcg_v1_debug_2_idx_i * R.local_num_col + bcg_v1_debug_2_idx_j;
...
void bcg_Scalapack_qr(csrType_local mat, denseType B, denseType X, double epsilon, int myid, int numprocs) {
    ...
    int context, desc[9], R_global_row, R_global_col,
            R_block_row, R_block_col, rsrc, csrc, lld,
            info, mype, npe, myrow, mycol, nprow, npcol;

    int *map = (int*) calloc(numprocs, sizeof (int));
    int map_idx;
